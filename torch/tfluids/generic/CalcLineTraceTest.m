...
%   CalcLineTraceSamplePlot(pos, delta, dims, obs);
  ...
  for dim = 1:3
    pos = dims / 2;
    pos(dim) = dims(dim) - 0.5 - 4.1; % Step back 4.1 units from the border.
    delta = [0, 0, 0];
    delta(dim) = 6.1;  % This should step us past the border.
    expected_pos = pos;
    expected_pos(dim) = dims(dim) - 0.5;  % Should hit the boundary.
    [collide, new_pos] = CalcLineTraceSamplePlot(pos, delta, dims, obs);
    assert(collide == 1);
  ...
  for dim = 1:3
    ...
    delta(dim) = -6.1;  % This should step us past the border.
    expected_pos = pos;
    expected_pos(dim) = -0.5;  % Should hit the boundary.
    [collide, new_pos] = CalcLineTraceSamplePlot(pos, delta, dims, obs);
    assert(collide == 1);
    assert(norm(new_pos - expected_pos) < 1e-4);
  end
  ...
  assert(abs(new_pos(1) - (width - 0.5)) < 1e-4 || ...
    abs(new_pos(2) - (height - 0.5)) < 1e-4 || ...
    abs(new_pos(3) - (depth - 0.5)) < 1e-4);
  
  % Case 4: Step off exactly the corner (just do one of them).
  pos = [width - 1.5, height - 1.5, depth - 1.5];
  delta = [2, 2, 2];
  expected_pos = dims - 0.5;
  [collide, new_pos] = CalcLineTraceSamplePlot(pos, delta, dims, obs);
  assert(collide == 1);
  assert(norm(new_pos - expected_pos) < 1e-4);
  
  % Case 4: Step off exactly a mixed corner (just do one of them).
  pos = [width - 1.5, 0.5, depth - 1.5];
  delta = [2, -2, 2];
  [collide, new_pos] = CalcLineTraceSamplePlot(pos, delta, dims, obs);
  assert(collide == 1);
  expected_pos = [width - 0.5, -0.5, depth - 0.5];
  assert(norm(new_pos - expected_pos) < 1e-4);
  
  % Case 5: Do a collision with the geometry.
  pos = [4.5, height - 4.2, 10.1];
  delta = dims / 3 - pos;
...
function [collide, new_pos] = CalcLineTraceSamplePlot(pos, delta, dims, ...
  ...
  % Note: all indices have been 0-indexed, but we need 1 indices and we
  % want (0, 0) to be the CENTER of the first pixel.
  pos = pos + 0.5;
  new_pos = new_pos + 0.5;
