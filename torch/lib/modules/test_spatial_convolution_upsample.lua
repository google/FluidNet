...
if nn.SpatialConvolutionUpsample == nil then
  dofile('spatial_convolution_upsample.lua')
end
...
function test.SpatialConvolutionUpsample()
  local batchSize = torch.random(1, 3)
  local nInputPlane = torch.random(1, 5)
  local nOutputPlane = torch.random(1, 5)
  local kW = torch.random(1, 5)
  local kH = torch.random(1, 5)
  local dW = torch.random(1, 5)
  local dH = torch.random(1, 5)
  local padW = torch.random(0, 5)
  local padH = torch.random(0, 5)
  local scaleW = torch.random(1, 4)
  local scaleH = torch.random(1, 4)
  local widthIn = torch.random(5, 10)
  local heightIn = torch.random(5, 10)

  local module = nn.SpatialConvolutionUpsample(
     nInputPlane, nOutputPlane, kW, kH, dW, dH, padW, padH, scaleW, scaleH)
  ...
  assert(torch.type(conv) == 'nn.SpatialConvolution')

  local input = torch.rand(batchSize, nInputPlane, heightIn, widthIn)
  local output = module:forward(input)
  -- Output size explained at:
  -- github.com/torch/nn/blob/master/doc/convolution.md#nn.SpatialConvolution
  local oHeightConv = math.floor((heightIn + 2 * padH - kH) / dH + 1)
  assert(oHeightConv == conv.output:size(3))
  local oWidthConv = math.floor((widthIn  + 2 * padW - kW) / dW + 1)
  assert(oWidthConv == conv.output:size(4))
  assert(oWidthConv * scaleW == output:size(4))
  assert(oHeightConv * scaleH == output:size(3))
  ...
  for i = 1, #paramsCPU do
    ...
    mytester:assertlt(err , 10 * precision, 'gradParams GPU error')
  end
end
